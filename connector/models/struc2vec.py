...
class Struc2Vec():
    ...
    def get_embeddings(self, ):
        ...
        self._embeddings = {}
        for word in self.graph.graph.nodes():
    ...
    def _get_order_degreelist_node(self, root, max_num_layers=None):
        ...
        visited = [False] * len(self.graph.graph.nodes())
        queue = deque()
        level = 0
        queue.append(root)
        visited[root] = True
        ...
        while (len(queue) > 0 and level <= max_num_layers):
            ...
            while (count > 0):
                ...
                degree = len(self.graph.graph[node])
        ...
        if os.path.exists(self.temp_path + 'structural_dist.pkl'):
            ...
            if self.opt2_reduce_sim_calc:
                ...
                for v in self.idx:  # c:list of vertex
                    nbs = get_vertices(
                        v, len(self.graph.graph[self.idx2node[v]]), degrees, n_nodes)
